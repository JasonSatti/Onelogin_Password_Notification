...
# -*- coding: utf-8 -*
"""
onelogin_notification - Notify users of Onelogin password expiration via
Slack DM.

Used to notify users via Slack DM if their Onelogin password is about to
...
class OneloginApi(object):
    ...
    def get_password_date(self, email):
        ...
        """
        headers = {"Authorization": f"Bearer: {self.token}"}
        r = requests.get(self.url, headers=headers, params={"email": str(email)})
...
def main():
    ...
    company_onelogin = OneloginApi(
        onelogin_info.oauth_url,
        onelogin_info.url,
        os.environ["ONELOGIN_ID"],
        os.environ["ONELOGIN_SECRET"],
    ...
    for user in company_onelogin.get_user_emails():
        ...
        if days_left <= 7:
            slack_notification.send_user_msg(user, days_left)
            print(f"Sent Slack DM: {days_left} days till expiration for {user}")
